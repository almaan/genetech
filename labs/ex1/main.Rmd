...
Welcome to the first lab! Here the objective is to get _you_ familiar with the
programming language `R`, and some of the commands that we will be using
throughout the remaining labs. For things to run smoothly in the more advanced
labs, please work through these examples and make sure you understand what is
going on - i.e, do not just click through the exercises. 
...
Stackoverflow and google are your friends, but they will not always be there for
you (sad but true). However, for most R functions, you will be able to get a
good overview of how to use them and how to apply them by typing:
`?function_name` or `help(function_name)`, e.g. `?sum()` in your R console.

## Objects and variables

Variables is one of the most standard features of any programming language; in
short you can think of them as placeholders. $pi$ is a great exampel of a
real-life variable, we use it whenever we want to indicate the value
$3.14159265...$. In the same spirit, we can assign values or objects a specific
name (e.g, fancy_variable), one huge benefit with this is that if we decide to
change the value of `fancy_variable` from say `3` to `4`, only the one instance
where we set the value of `fancy_variable`, the rest of the code remains intact.

In R, you have 2 options when you want to bind an object or value to a name
`var_name <- value` and `var_name = value`. The best practice is to always use
`<-`, unless the right hand side is a mathematical expression (where an equal
...
Computations and manipulations of numbers are integral parts of any programming
language, hence it's essential to know how to call for these kind of actions.
Below you will see some examples of the perhaps most common mathetmatical
...
7 - 4  # => 3 (subtraction)
7 / 2  # => 3.5 (division)
7 * 2  # => 14 (mulitplication)
2 ^ 3  # => 8 (exponents)
7 %% 3 # => 1 (modulo)
```

As in standard mathematics parenthesises are used to indicate the order by which
...
There is a plethora of different mathematical functions in R, usually their
names resemble those of the mathematical operations that they represent, for
example:
...
## Logical operators
Once calculations have been performed using the different arithmetic operations,
we usually want to assess and _compare_ our results, this is when logical
operators come in handy; when used, they return a value that is either `TRUE` or
`FALSE` depending on the expression you wrote down. Some of the most useful logical
operators are:

* `==` : equal to - `TRUE` if LHS is equal to RHS
* `>` : greater than - `TRUE` if LHS is greater than RHS
* `>=` : greater than or equal to - `TRUE` if LHS is greater or equal to RHS
...
There are also ways of linking logical expressions, creating a compund
expresson. For example, you could check whether two expressions are true at the
_same_ time. The two variants that you will mainly encouter in these labs are:
...
### Data Classes
Every (programming) language has it's own classes of objects, you have actually
already worked with several different ones just in this lab! Just to get you
familiar with the terminology, some of the _atomic classes_ (that build up every
other class) are:
...
complex value, the numeric is converted to a complex type and all is good:
...
Java) with this coercion, you should also be cautiuos - sometimes unexpected
results and later on errors may be introduced. Take a look at what happens when
...
What? This makes no sense at all, how can we even compare a character with a
number? Shouldn't this, if something, raise an error?

The answer is that the numeric `3` gets coerced into a character `"3"` which is
very much comparable to another character (if you, like in R, has a assigned an
order to the characters)

## Arrays

The next element to introduce is _arrays_, these are fundamental structures that
you will use *a lot*. In a nutshell, arrays let us associate multiple values to
a single object. We'll be focusing on 1 and 2 dimensional arrays, let us start
with 1d arrays first.

### 1d arrays - lists and vectors

1d arrays come in two flavors, either as lists or as vectors. Which you create
accordingly:

```{r,eval=TRUE, echo=TRUE}
a_list <- list(20200504,
               "X AE A-Xii",
               0,
               "cat")
...
a_list_2nd_element <- a_list[[4]]
a_vector_2nd_element <- a_vector[4]

print(paste("The fourth element of our list is : ",a_list_2nd_element))
print(paste("The fourth element of our vector is : ",a_vector_2nd_element))
 
```


We could also assign "names" to our array elements:

```{r,eval=TRUE, echo=TRUE}
a_list <- list(birth_date = 20200504,
               name = "X AE A-Xii",
               problems = 0,
               favorite_animal = "cat")
...
We can still access our elements by specifying their position, but now also by
their names; sometimes, this allows you to write more readable code and keep
better track of what elements you access. 

To access an element by name, for _lists_ use the `$` symbol or `[['name']]`;
for vectors only the second option is valid. To illustrate:
...
There's a nifty command called `names` that we can use
to check the names of our list and vector elements respectively:
...
* `a_vector[c(name_1,name_2)]` - creates a new vector with elements "name_1" and "name_2"

As an example:

```{r, echo =TRUE, eval=TRUE}
a_list_subset <- a_list[1:3]
a_vector_subset <- a_vector[c("slot_1","slot_2","slot_3")]
...
Vectors are *homogenous* while lists can be *heterogenous*.

This means that all the elements in a _vector_ must be of the same type, and if
...
had we we used a list insted, the output would have been different:
...
a_list_of_mixed <- list(list_1 = list("hello",13,TRUE),
                        list_2 = list(45,"covfefe"),
                        vector_1 = c("sheep","cow","dog")
                        )
```

*NOTE* : This functionality does not exist for vectors
...
that constitute its elements (second dimenson). When the lists or vectors that
we want to store are of different length, using a list is the best
option, *BUT* if they are of equal length (structured) there are better options
to choose from, being the `matrix` and `dataframe` types.
...
As eluded to above, when we have structured or _tabular_ data there are two data structures that are particularly useful:

* Matrices - for homogenous data
* Dataframes - for heterogenous data
...
a_df <- data.frame(birth_date = c(20200504,19691294),
                   first_name = c("X AE A-Xii","Jay"),
                   problems = c(0,99),
                   favorite_animal = c("cat","tiger"),
...
Just as we could check the names of the elements in our lists and vectors, we
can do the same for the columns and rows in our 2D arrays, but now we use the
functions `colnames` and `rownames` instead.
...
The same conventions can be used to access elements in a data frame, but you can
...
frame. To extract a column and preseve its row/colnames do  `a_df["col_name"]`.
...
so that is interesting, but what if we wanted the max value in each column and
minimum value in respective row, how do we do this?  One possibility would be to
: isolate each row/column, compute the max/min value manually and finally store
these values in a vector. That's a pretty messy and ugly solution, luckily we
have the `apply` function to work with instead!
...
The help section may a bit hard to interpret, but in short it says that if we
...
Let's say that we want to compute the max value of each column in `a_matrix`, then we have:

* `X` : `a_matrix`
* `MARGIN` : 2
* `FUN` : `max` (note we do not _call_ `max` here, only provide the name)

and in action this becomes:
...
have been included as standard functions in R, meaning you
don't have to use `apply` but can call them directly. Some examples are:
...
As you can see the results are similar to if we used `apply`:
...
Varibles allow you to specify a certain value only _once_ instead of every time
you use it, functions allow you to do the same but for _actions_. In short, you
associate a certain action to a function, and whenever you want to execute that
action, you simply call the function. We call the input to our functions
...
volume of sphere given the radius, the mathematical formula for this is:
...
each iteration, executes a certain code block. The "skeletton" of a for loop is
as follows:
...
here `vec` would be the array we iterated over. To give a less abstract example, we can define a vector that contains all numbers from 1 to 5, done by `vec <- c(1:5)` and then in each iteration print the value we are at:
...
the elements in our arrays do not have to be numbers or in any specific order:
...
Implementing this in could could for example look like:
...
To give you a somewhat less abstract example, run the chunk below _a couple of
...
Create a data frame (named `count.data`) with 3 samples and 5 genes. Make all count values = 1
...
generated, each element in the vector is one base in the sequence. You can
do not have to pay attention to this particular code.
...
`sequence`. This vector could, for example, look like `c(A = 11, C = 132, T = 761, G = 41)`.
...
and `.tsv` respectively) one way of doing this is by using the `read.table`
function.

If you fields are separated by commas (as in a `.csv`file ) use the argument
...
Sooner or later you will come to a point where you want to visualize your
results, since this is usually helpful for you and others to interpret them. We
will therefore explore how we may plot data in R.

We can generate a simple scatterplot by using the `plot()` function.
...
TLDR: it is a "collection of bioinformatic tools".
...
There are **a lot** of "packages" within bioconductor that you can install
and use. To install a package from bioconductor use `BiocManager::install(PACKAGE_NAME)`
where you replace `PACKAGE_NAME` with the package of interest, e.g.
...
given sample you may do so for both the meta-data and assay in one operation,
which ensures the meta-data and observed data will remain synchronized.

Flawed handling of meta and observational data has resulted in a number
of incorrect results and retractions so this is a very desirable property.
...
```{r, echor =TRUE, eval = TRUE, message=FALSE, warning=FALSE}
library(SummarizedExperiment)
```

Next we load a second library that contains our data called `airway`:
...
**How :**  Go to the top of this document, set the variable (on line 14)
`GRADE_MODE` to `TRUE`. Knit this document, and hand in the resulting html-file
on Canvas. Make sure you have solved all the questions. If the document doesn't
compile, there is something wring in the edited code.
