...
set the value of `fancy_variable` will have to be changed rather than all places
where it is used.

In R, you have 2 options when you want to bind an object or value to a name
`var_name <- value` and `var_name = value`. For assignment, the best practice is to always use
`<-`, this is in line with Google's R Style Guide (widely used).
...
Below you will see some examples of the more common mathetmatical
...
already worked with several different ones in this lab! Just to get you
familiar with the terminology, some of the _atomic classes_ (that build up every
other class in R) are:
...
Say What? This makes no sense at all, how can we even compare a character with a
number? Shouldn't this, if something, raise an error?

The answer is that the numeric `3` gets coerced into a character `"3"` which
_can_ be compared to another character (if you, like in R, have assigned an
...
1d arrays come in two flavors, either as _lists_ or as _vectors_. Which you create
accordingly:
...
* Dataframes - for heterogeneous data
...
One possibility would be to: isolate each row/column, compute the max/min value
manually and finally store these values in a vector. However, that's a pretty messy and
ugly solution, luckily we have the `apply` function to work with instead!
...
here `vec` would be the array we iterated over. To give a less abstract example,
...
results. Visualization is usually helpful for you and others to interpret the
results. We will therefore explore how we may plot data in R.
...
TLDR: it is a "Bioconductor is a collection of bioinformatic tools".
...
given sample you may do so for both the count and meta-data in one operation,
which ensures the meta-data and observed data will remain synchronized.

Bad coordination of meta-and observational data has resulted in a number of
...
To access a particular sample's data, just type e.g. `se$cell` to find the cell data. 
