...
actions. Below you will see some examples of the more common mathetmatical
...
Java) with this automatic coercion, you should also be cautiuos - sometimes unexpected
...
What's happening here is that the numeric `3` gets coerced into a character
`"3"` which _can_ be compared to another character (if you, like in R, have
assigned an order to the characters.

## Arrays

The next elements to introduce are _arrays_, these are fundamental structures
...
Now, we can still access our elements by specifying their position, but now also by
