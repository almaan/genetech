...
output:
  html_document:
    ...
    theme: "readable"
---
<!-- Lab creator: Alma Andersson, Ludvig Larsson -->
...
Welcome to this lab, where we will explore spatial transcriptomics data. While it
...
2018, 10x Genomics acquired the IP rights to the ST-technique, they then launched
...
printed at locations (spots) arranged in a hexagonal grid. These oligonucleotides all have 
...
mRNA moleculas can diffuse freely from the cells and a fraction of these molecules 
...
breast cancer, a disease claiming several thousand lives every year. 10x Genomics - the
...
```{r load_img, fig.width=8, fig.height=8}
...
> Now when the Seurat object is formed, please answer the questions below:
>
...
>
> *hint* : You can use the function `GetAssayData()` to retrieve to expression matrix
> (i.e. UMI counts).
...
# the variables ans1, ans2, ans3 and ans4
...
```{r plot_outline, fig.width=6, fig.height=6}
...
> Use `ST.FeaturePlot()` to visualize the number of unique genes ("nFeature_RNA") per
> spot. Add an argument palette = "Spectral" to get a more fancy color scale.

```{r q2, fig.width=6, fig.height=6}
...
```{r plot_counts, fig.width=6, fig.height=6}
...
```{r plot_erbb2, fig.width=6, fig.height=6}

# You can play around with the pt.size and pt.alpha (opacity) to make the background more 
# visible
FeatureOverlay(se.HER2, features = "ERBB2", pt.size = 1.5, pt.alpha = 0.7)
...
```{r load_annot_img, fig.width=6, fig.height=6}
...
> Armed with the `FeatureOverlay()` function, it's now up to you to find out which
> **one** of these genes below, that are highly expressed in in one or more of the
> *DCIS* regions while less prominently expressed in the remaining part of the tissue.
>
> -   ESR1
> -   FSCN1
> -   MGP
...
> -   ACTA1
>
> Use the `FeatureOverlay()` function inspect the spatial distribution of these genes 
> and compare to pathologists annotated image above. Once you have your answer, plot 
> that gene only!

```{r q3, fig.width=7, fig.height=6}

# Plot the gene expressed mainly in the DCIS region here
# using the FeatureOverlay() function

```

Write down the gene expressed mainly in the DCIS region and check your answer.

```{r check_q3, warning=TRUE}
# Replace NA with the gene highly expressed in the DCIS region here
ans.q3 <- NA
...
```{r plot_hist, fig.width = 14, fig.height = 5}
...
> Create a vector named `keep.spots` which contains the *names* of all spots with
> **both** of the following criteria satisfied:
>
> 1.  `nCount_RNA` should be larger or equal to $2000$
> 2.  `nFeature_RNA` should be larger or equal to $800$
>
> *Comment* : Do *not* make any changes to your Seurat object yet.

```{r q4}

# Enter code to create the keep.spots vector here
# Replace NA for the code that generates the requested output.
keep.spots <- NA
```

Now you can check your answer below before you proceed!

```{r check_q4, warning=TRUE}
...
```{r log_hist, fig.width=14, fig.height=5}
...
> `keep.genes` with the **names** of the genes that satisfy both the following criteria:
>
> 1.  Total expression across all spots is larger or equal to $300$
> 2.  The gene should be observed (i.e., have a non-zero count) in *at least* 5 spots
...
> Subset your Seurat object `se.HER2` using the vectors `keep.spots` and `keep.genes`
> with the `SubsetSTData()` function. The subsetted Seurat object should still be named
> `se.HER2` (i.e., you should overwrite the old one). `SubsetSTData()` is a function
> provided in `STUtility` which can take a vector of spots that should be passed to the
> `spots=` argument and a vector of genes that should be passed to the `features=`
> argument.
>
> NOTE: You should *not* use the `subset()` function that was used in the previous lab.
> You can find more information in the `SubsetSTData()` documentation by typing
> `?SubsetSTData` in the console.

```{r q6}

# enter code to subset your Seurat object in place of "..."
se.HER2 <- SubsetSTData(...)
...
> Now please answer these to questions below:
>
...
```{r sctrans, echo=FALSE, eval=TRUE}
...
```{r plot_clust, fig.width=6, fig.height=6}
...
> To make sure you aren't just scrolling through this exciting stuff, let us throw in a
> question! In the chunk below, answer the following two questions:
>
...
```{r plot_markers,fig.width = 10, fig.height=9}
...
> Create a vector called `factorization.genes` that includes the top 1000 most variable
> genes.
>
> *Comment* : The top variable genes were already identified during the normalization
> step with `SCTransform()` and we can get those by running `Variablefeatures(se.HER2)`.

```{r q9}

# Enter the code here
# Replace NA for your answer

factorization.genes <- NA

# Print out first 10 entires
head(factorization.genes)
```

Answer check.

```{r check_q9, warning=TRUE}
...
```{r plot_nnmf, fig.width=12, fig.height=8}
...
> What are the 10 spots with the highest contribution to factor 10? What are the 10
> genes with the highest contribution to factor 3?
>
> Give your answers as comments in the code chunk below, they should be short and
> concise, not more than a sentence or two.
...
# print answers
cat(sprintf("The spots with the highest contribution to factor 10 are: \n%s", paste(q10.ans1, collapse = "\n")), "\n\n")
...
So what we see here is *a lot* of genes whose names start with *IG*.

### Q11

> It's often common to google a few of the genes to get a "sense" of what kind of
> biological process(es) each factor could represent. And, evaluation of results are
> just as important as obtaining them. Thus your task is to -- with the help of the
> internet if you like -- pick the biological process from the ones listed below that
> you consider most likely that factor 7 is associated with.
>
> **Processes**
>
...
> Create a data frame called `top.2.genes` with the *2 genes* that are most strongly
> associated with each factor. This data frame should have the columns: "gene", "weight"
> and "factor", containing the following information:
>
> -   gene - name of the genes
> -   weight - gene contribution from the associated factor
> -   factor - the factor to which the gene is associated (these should be called
>     "factor_1", "factor_2", ...)
>
> *Hint 1*: The names of the genes can be accessed via `rownames(W)` and the names of
> the factors can be accessed via `colnames(W)`. The factor contribution scores are the
> numerical values found in *W*.
>
> *Hint 2*: It might be a good idea to use a for-loop here. However, it's not a must,
> there are multiple ways of solving this exercise!
>
> *Hint 3*: If you want append an element to a vector, you simply do
> `my.vector <- c(my.vector, new.element)`
>
> *Hint 4*: The `data.frame` should look something like this: 
> ![](./img/data_frame.png)

```{r q12}

# code to produce the top.2.genes data.frame
# replace NA with your the final data.frame
top.2.genes <- NA


# Print out answer for reference
top.2.genes
```

And you guessed it, you can check your answers with the chunk below!

```{r check_q12, warning=TRUE}
...
```{r plot_cells, fig.width=12, fig.height=4}

# specify cell types to plot
selected.cell.types <- c(
  "B.cells",
  "T.cells",
  "Epithelial"
)
...
```{r plot_correlation, fig.width=6, fig.height=6}
...
> Using the correlation matrix, please complete the following sentences
>
> 1.  Myeloid cells strongly anticorrelate with...
> 2.  T-cells strongly spatially co-localize with...
>
> *Comment* use the following cell type names when answering below: "T-cells",
> "B-cells", "Epithelial", "SMCs","Myeloid", "Endothelial","CAFs", and "Plasmablasts"
> *Hint*: Look at the colorscale, which end represents correlation versus anticorrelation?
> *Hint2*: For 2, we are not interested in "T-cells" although they perfectly co-localize for some "strange" reason ;)
...
```
Let's check that the final answer is correct.

```{r check_q13}
source("checks.R")
q13_check(ans = list(ans.q13.1, ans.q13.2))
```
