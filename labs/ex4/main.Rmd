...
coordinates <- spatial.data[[]][, c("x", "y", "pixel_x", "pixel_y")]
...
ST.FeaturePlot(se.HER2, features = "orig.ident")
...
there is a small scale bar above the tissue that helps you to figure out distances in the plot,
...
# ST.FeaturePlot(se.HER2, features = "nFeature_RNA", palette = "Spectral")
...
function. We can then plot our H%E image(s) with the `ImagePlot()` function.
...
FeatureOverlay(se.HER2, features = "ERBB2", pt.size = 1.5)
...
*Q3*: Armed with the `spatial.plot` function, it's now up to you to find out which (choose one) 
...
# FeatureOverlay(se.HER2, features = "MGP")
...
# generate hisograom of UMI counts
...
keep.spots <- intersect(colnames(se.HER2)[se.HER2$nCount_RNA >= 2000],
                        colnames(se.HER2)[se.HER2$nFeature_RNA >= 800])
...
keep.genes <- rownames(se.HER2)[gene.data$nCounts >= 300 & gene.data$nObs >= 4]
...
`keep.genes` using the `SubsetSTData()` function. The subsetted Seurat object 
...
se.HER2 <- SubsetSTData(se.HER2, spots = keep.spots, features = keep.genes)
...
ans.q7.1 <- ans2 - nrow(se.HER2)
ans.q7.2 <- ans1 - ncol(se.HER2)
...
factorization.genes <- VariableFeatures(se.HER2)[1:1000]
...
**Q10**: 
What are the 10 spots with the highest contribution to factor 7? 
...
q10.ans1 <- rownames(H)[order(H[, 7], decreasing = T)[1:10]]
q10.ans2 <- rownames(W)[order(W[, 3], decreasing = T)[1:10]]
...
Factor 8 looks kinda interesting, almost as if it's surrounding the DCIS
...
FactorGeneLoadingPlot(se.HER2, factor = 8)
...
print(sprintf("Factor 6 is associated with process %d", ans.q11))
...
top.2.genes <- data.frame()
genes <- rownames(W)

for (i in 1:10) {
  top.genes <- genes[order(W[, i], decreasing = TRUE)][1:2]
  top.scores <- sort(W[, i], decreasing = TRUE)[1:2]
  top.2.genes <- rbind(top.2.genes, data.frame(gene = top.genes, weight = top.scores, factor = paste0("factor_", i)))
}
