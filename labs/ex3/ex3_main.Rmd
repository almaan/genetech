...
Welcome to Lab 3, where we will explore spatial transcriptomics data. While it
...
```{bash,echo=F,eval=F}
wget https://cf.10xgenomics.com/samples/spatial-exp/1.1.0/V1_Breast_Cancer_Block_A_Section_1/V1_Breast_Cancer_Block_A_Section_1_filtered_feature_bc_matrix.tar.gz -O /home/genetech/code/labs/ex3/data/bc-data.tar.gz -nv
tar -xf data/bc-data.tar.gz -C data/
rm data/bc-data.tar.gz

wget https://cf.10xgenomics.com/samples/spatial-exp/1.1.0/V1_Breast_Cancer_Block_A_Section_1/V1_Breast_Cancer_Block_A_Section_1_spatial.tar.gz -O /home/genetech/code/labs/ex3/data/spatial.tar.gz -nv
...
1. The second-to-last and last columns in `tissue_positions_list.csv` represent the _x_ and _y_ coordinates respectively. You can ignore all other columns except the barcodes of course. Note that the columns are not names, so you will have to do this yourself.
...
# read file, setting the first column (barcode) as rownames
spatial.data <- read.csv(file = "data/spatial/tissue_positions_list.csv", header = FALSE, row.names = 1)
# Select last two columns
spatial.data <- spatial.data[, 4:5]
# Rename columns to "x", "y"
colnames(spatial.data) <- c("x", "y")
# Find intersecting spots
intersecting.ids <- intersect(colnames(raw.data), rownames(spatial.data))
# Reorder spatial.data using intersecting spots
spatial.data <- spatial.data[intersecting.ids, ]
# Reorder raw.data using intersecting spots
raw.data <- raw.data[, intersecting.ids]
# _______________________________


# Suggested solution 2 - slightly more compressed solution
# _______________________________
spatial.data <- setNames(read.csv(file = "data/spatial/tissue_positions_list.csv", header = FALSE, row.names = 1)[, 4:5], nm = c("x", "y"))
spatial.data <- spatial.data[intersect(colnames(raw.data), rownames(spatial.data)), ]
raw.data <- raw.data[, intersect(colnames(raw.data), rownames(spatial.data))]
# _______________________________
...
_Comment1_ : Note how we set the `spatial.data` as meta data in our Seurat object.
...
se <- CreateSeuratObject(counts = raw.data, 
                         meta.data = spatial.data, 
                         project = "BreastCancer", 
                         assay = "spRNA")
...
ans1 <- ncol(se) # or dim(se)[2]
ans2 <- nrow(se) # or dim(se)[1]
ans3 <- sum(se$nCount_spRNA) # or sum(GetAssayData(se, slot = "counts"))
ans4 <- rownames(se)[1306] # or rownames(GetAssayData(se, slot = "counts"))[1306]

cat(sprintf("No. Spots : %d \nNo genes: %d\n1306th gene : %s",ans1,ans2,ans3))
q1.ans <- c(ans1,ans2,ans3,ans4)
```

The code below will allow you to check whether your answer is correct or not.
...
1. The data we have collected seems to be of descent quality, nothing weird (like mixing up the spatial barcodes) has happened to it.
2. Our code for plotting also seems to be correct!
...
and see if you follow the workflow.
...
create the same plot as before, using the `color.vector` from earlier (expression of _ERBB2_)
to color our spots.
...
color.vector <- as.numeric(se$spRNA["MGP",])
gg <- spatial.plot(se,
                   img = img,
                   color.vector = color.vector
                   )
...
keep.spots <- colnames(se)[se$nCount_spRNA >= 2000 & se$nFeature_spRNA >= 800]
```
...
2. The gene should be observed (i.e., have a non-zero count) in _at least_ 5 spots (about 0.2% of )
...
# Alternative 1 - Here I used the nCount.UMI defined earlier which is not part of gene.data
keep.genes <- rownames(se)[nCount.UMI >= 300 & gene.data$nObs > 5]

# Alternative 2 - remember that the nCounts in gene.data are log10-transformed
keep.genes <- rownames(se)[gene.data$nCounts >= log10(300 + 1) & gene.data$nObs > 5]

# Alternative 3 - tidyr solution with pipes
keep.genes <- gene.data %>%
  filter(nCounts >= log10(300 + 1), nObs > 5) %>% 
  row.names(.)
...
# Alternative 1
se <- subset(se, cells = keep.spots, features = keep.genes)
...
ofdimensionality](https://en.wikipedia.org/wiki/Curse_of_dimensionality). Here
...
One way to further explore the cluster is to do a DGE (Differential Gene Expression)
analysis. Again, we'll reuse some functions from the single cell lab to do
...
top.5000 <- names(sort(Matrix::rowSums(GetAssayData(se)), decreasing = TRUE)[1:5000])
# print head of top.5000
print(head(top.5000))
```

To check your answers, run the code chunk below!
...
Now, compute the row sums of the `gamma` slot in `lda.res`, inspect your result and answer the following questions:

4. The row sums are all identical, what is the value that all of them share?
...
gamma.row.sums <- rowSums(lda.res@gamma)
summary(gamma.row.sums)
...
# 1
# Question 10.5
# The rows sums of the gamma matrix represent the sum of proportions for each 
# topic which is always eaual to 1.
...
ones listed below that you consider most likely that factor 1 is associated
with.
...
ans.q11 <- 3 # IG* genes are all examples of immunoglobulins
...
factor. This data frame should have the columns: "gene","prob" and "factor",
...
_Hint 1_: the names of the genes can be accessed via `lda.res@terms`, as you saw
...
# Fancy solution using do.call, rbind, lapply
# I don't think any of the students will come up with this solution :-/
top.2.genes <- do.call(rbind, lapply(1:10, function(i) {
  x <- lda.res@beta[i, ]
  term.order <- order(x, decreasing = TRUE)
  data.frame(gene = lda.res@terms[term.order[1:2]],
             prob = exp(x[term.order[1:2]]),
             factor = i)
}))

# Alternative using for loop
# Much more likely that the students will do something like this
gene <- c() # Define empty vector
prob <- c() # Define empty vector
factor <- c() # Define empty vector
for (i in 1:10) {
  x <- lda.res@beta[i, ] # get beta values for factor i
  term.order <- order(x, decreasing = TRUE) # Order by decreasing probability
  gene <- c(gene, lda.res@terms[term.order[1:2]]) # append gene names
  prob <- c(prob, exp(x[term.order[1:2]])) # append probability values
  factor <- c(factor, rep(i, 2)) # append factor i
}
top.2.genes <- data.frame(gene, prob, factor) # Assemble vectors into a data.frame


# Alternative 2 using for loop and rbind
top.2.genes <- data.frame() # Define empty data.frame
for (i in 1:10) {
  x <- lda.res@beta[i, ] # get beta values for factor i
  term.order <- order(x, decreasing = TRUE) # Order by decreasing probability
  # bind new data.frame to existing top.2.genes data.frame
  top.2.genes <- rbind(top.2.genes, 
                       data.frame(gene = lda.res@terms[term.order[1:2]],
                                  prob = exp(x[term.order[1:2]]),
                                  factor = i))
}
...
challenges to this approach. The factors (a more general term than topic), do
not necessarily represent specific cell types and we have no immediate
biological interpretation of them.
