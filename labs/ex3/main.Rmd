...
# Lab - Single cell RNA seq (scRNA-seq)
...
```
Lets check if the answer is correct.

```{r check_q1, warning=TRUE}
source("checks.R")
check_q1(number.of.genes, number.of.cells)
```
...
n.cells.filt <- NA
n.genes.filt <- NA
sprintf("Number of cells after filtering: %s", n.cells.filt)
sprintf("Number of genes after filtering: %s", n.genes.filt)
```

Let's check that everything is correct before we proceed.

```{r check_q6, warning=TRUE}
source("checks.R")
check_q6(n.genes.filt, n.cells.filt)
```
...
```{r pcaplot1, fig.width=4, fig.height=3}
...
```{r pcaplot2, fig.width=4, fig.height=3}
...
```{r UMAP2, fig.width=8, fig.height=4}
...
```{r clustering, fig.width=4, fig.height=3}
...
Let's check that you got this right before continuing.
...
```{r clusters_q, fig.width=8, fig.height=4}
...
Now let's confirm that you got the correct number of clusters.

```{r check_q8, warning=TRUE}
source("checks.R")
check_q8(nClusters.0.3, nClusters.1.2)
```
...
Report the `term_name` and `p_value` below.
...
stopifnot("'term_name' is NA" = !is.na(term_name), "'p_value' is NA" = !is.na(p_value))
...
```{r check_q11, warning=TRUE} 
source("checks.R")
check_q11(term_name, p_value)
```
...
> What are the four cell types with the fewest cycling cells? The answer should be a character vector with 4 cell type names.
>
> _Hint_: Cells in the S, G2 and M phase are considered cycling. More information about cell cycling can be found on [this seurat vingette](https://satijalab.org/seurat/archive/v3.1/cell_cycle_vignette.html).

```{r q13}
# Replace the NAs with your answer
...
```{r check_q13, warning=TRUE}
source("checks.R")
check_q13(q13.answer)
```
