...
```{r, eval=FALSE}
metadata <- read.table(file = "tung/tung_metadata.tsv", stringsAsFactors = FALSE)
head(metadata)
```
...
should cosist of genes symbols.
...
In this course, we will use the [SCTransform](https://www.biorxiv.org/content/10.1101/576827v2) method
...
reasonable to expect that there esxists some set a co-expressed genes which defines this cell type. 
For example you might find that this cell type express genes related to neuronal development. 

In reality, an expression matrix represent many complex interactions between genes and these 
interactions can vary across different cell types and cell states. But there are typically clear 
patterns in the data and what we want to do is to make sense of these patterns. 
One major benefit with scRNA-seq data is that we can fish out these patterns using machine learning 
...
such as signalling pathways, immune functions, proliferative activity, apoptotic activity and much more. The 
interpretation of dimensionality reduction results can be confusing, but it can be helpful to think of them as 
patterns representing some biological activity (or technical artefacts).

For scRNA-seq data, these methods are absolutely crucial to visualize the data!

Let's have a look at the most widely used dimensionality reduction method called 
...
blood mononuclear cells (PBMCs). When you run `pbmc <- readRDS("tung/pbmc")` 
you will get a Seurat object which has already been normalized. PBMCs contain 
mainly immune cells such as lymphocytes and monocytes.

```{r}
pbmc <- readRDS("tung/pbmc")
```
...
Here you can already see some interesting structure in the data whcih represent
different cell type populations. Right now we don't 

_____
## 7) Clustering 
_____

Clustering refers to methods used to group samples (cells) based on gene expression
similarity (https://www.nature.com/articles/nmeth.4299). The methods are typically 
unsupervised, meaning that we can define group structures in the data without knowing 
anything about the data beforehand. It's important to keep in mind that clustering
methods will find structure in the data, whether it makes sense or not. In the case 
of scRNA-seq analysis, you are typically interested in defining cell types. However 
you need to characterize the clusters yourself to see if they make sense or not
which can be a very challenging task! 

Let's try clustering our data!

For this, we will use k-means clustering, which is a standard approach to clustering.
The k-means method that we will use here starts by assigning each cell to k groups 
which are randomly initiated. Therefore the algorithm is non-deterministic, 
meaning that we may get a different outcome each time we run it. To avoid this we 
can use the `set.seed()` function which will ensure that the initiation process will
remain the same every time we run it.
...
count matrix first! Also, have a look at the documentation by typing 
`?kmeans` in the R console.

```{r, eval=FALSE}

# Write your code / answer here
set.seed(123)
km <- kmeans(x = t(GetAssayData(se, slot = "counts")),
             centers = 5,
             iter.max  = 10,
             nstart = 10)
