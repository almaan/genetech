...
When you run `bc <- readRDS("data/bc")` you will get a `Seurat` object with some additional meta data included. 
...
Before we proceed, we will normalize the data with `SCTransform()` and run principal component analysis. This step might take a minute or two.

```{r bc_norm_and_pca, message=FALSE, warning=FALSE}
