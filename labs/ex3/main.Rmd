...
Now we can see that there is some separation of the cells based on individual. Exporing these types
...
NA19098 differ from the other two individuals. You can also obtain a heatmap visualization showing 
what genes that are driving this separation using the `DimHeatmap` function:

```{r}
DimHeatmap(se, dims = 2)
```
...
similarity (https://www.nature.com/articles/nmeth.4299). Cluster obtained from scRNA-seq data
are typically interpreted as cell types or cell states. The methods are typically unsupervised,
meaning that we can define group structures in the data without knowing anything about
the data beforehand.
