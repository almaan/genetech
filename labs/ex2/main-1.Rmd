...
For this second lab we will be working with bulk-RNA sequencing data, a method that became a gold-standard for transcriptomic research that was published in the mid 00s. Although the method has been around for a while it is still widely used to by research labs due to its robustness and simplicity.
...
> One of the most important aspect doing data analysis is to understand what kind of data types you are using and the dimensions of these data modalities.
>
> 1.  What value type do you have in `counts`?
>
> 2.  What is the dimensions of `counts`?
>
> *Hint*: Use commands from lab 1.

```{r Q1}
# Replace NA with correct answer

q1.ans1 <- NA
q1.ans2 <- c(rows = NA, cols = NA)
...
> Add a new column to the existing `gg` data.frame with log10-transformed gene expression values for the gene `COL1A1`. Use it to plot a new histogram with these values.
...
> Create a boxplot with log10 transformed counts for each sample and color the samples by their cancer subtype.
>
...
> Make the same plot as in Q3 but using `facets` instead of colors.
>
...
Pre-processing of raw fastq data: \* Check quality of raw reads and filter out reads with poor quality \* Filter raw reads to remove artifacts (e.g. Illumina primer sequences and adapters) \* Align reads to a reference genome (or construct a *de novo* genome assembly if a reference is unavailable) \* Annotate aligned reads to gene loci \* Quantify read counts per gene loci
...
Quality control: \* Check quality of samples and filter out outliers \* Filter out low abundant genes or other potentially confounding genes \* Adjust for other confounding factors, e.g. gender specific expression, experimental date and age

Analysis: \* Differential gene expression analysis \* Data transformations for visualization and clustering \* Clustering \* Gene regulatory networks
...
> Filter the existing `counts` matrix to exclude all genes in that have a total count across all samples of less than or equal to 100. Use the skills you learned in Lab 1.
>
...
> Make a boxplot for each sample showing the distribution of normalized counts for each sample grouped by subtypes.
>
...
> 
> _Hint_: You should see very distinct difference in the heatmap between the the samples in `basal` versus those in `normal` for both the up- and down-regulated genes.
...
> This heatmap showes the top 20 differentially upregulated genes for each cancer subtype. What conclusions you can make from this plot based on the differences between subtypes? Write max 2 sentences.
>
> *Hint*!\_: Look how clear the subtype specific genes are and variability between groups.
