...
For this part of the lab we are exploring RNA-seq data from breast cancer samples 
obtained from The Cancer Genome ATlas (TCGA) which have been classified into five 
different "mRNA subtypes"; Her2, Basal, LumA, LumB and Normal. The full dataset consists 
of 1217 patient samples but for the purpose of this exercise, we have reduced this 
dataset to 10 patients per subtype.
...
#Write code
se <- se[rowSums(assays(se)$counts) > 100, ]
...
colnames(exprMat) <- colData(se)$subtype
...
gg <- data.frame(subtype = colData(dds)$subtype, val = dds@assays@data$counts["NNMT", ])
