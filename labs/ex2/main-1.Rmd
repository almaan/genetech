...
# conv <- c("Basal_TP" = "Basal-like", 
#           "Her2_TP" = "Her2",
#           "LumA_TP" = "LumA",
#           "LumB_TP" = "LumB",
#           "Normal_NT" = "Normal",
#           "Normal_TP" = "Normal-like")
# metadata$subtype <- conv[metadata$subtype]
# write.table(x = metadata, file = "~/genetech/labs/ex2/data/TCGA_breast_cancer_subtypes_metadata.tsv", sep = "\t", quote = FALSE, row.names = TRUE, # col.names = TRUE)
...
This next code chunk might take a few minutes to run.
...
We do find quite many subtype specific genes in our data, but we as you can see even the top most 
differentially expressed genes have a quite high within group variability. Even subtypes of cancer 
are heterogeneous and remember that these are bulk RNA-seq data which means that we are investigating 
the averaged expression over a whole tissue piece. Cancer cells reside and proliferate within a 
microenvironment made up of "normal" cells, for example cells that build up connective tissues and blood
vessels or immune cells that are trying to fight and eliminate the tumor. Although cells in the tumor
...
This is to a large extent the reason why single-cell and spatial methods have taken over the field 
of genomics and gained so much traction in the last decade.
