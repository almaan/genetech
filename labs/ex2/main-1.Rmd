...
cohort and have been classified into 5 different "mRNA subtypes"; Her2, Basal-like, LumA, LumB and 
Normal-like. We have also included a sixth "subtype" which consists of normal tissue to be used as a control. 
The full cohort consists of 1217 patients but for the purpose of this exercise, we have reduced this dataset 
to 10 patients per subtype, adding up to a total of 60 samples.
...
We should also warn you that these subtypes are perhaps a bit outdated but were defined in the original TCGA [paper](https://www.nature.com/articles/nature11412) on breast tumors in 2012. Here for example, they use TNBC
as a synonym for basal-like cancers and although these two types are similar, they are not biologically equivalent. 
...
specific package, use `BiocManager::install("packageName")` if the package is 
hosted on bioconductor or `install.packages("packageName")`if the package is
hosted on CRAN. When running this, you often get asked "Update all/some/none? [a/s/n]:" 
In general, it's ok to skip updating the rest of the packages for this lab (which can 
...
ggplot2 is a graphics framework which has became very popular among R users, mainly because its aesthetically pleasing nature. 
...
Q3 - Now make the same plot but with facets instead of colors.
...
pre-processing of raw fastq data:
  ...
  * Filter raw reads to remove artifacts (e.g. Illumina primer sequences)
...
These steps are typically done using command line tools and will not be covered in this
lab. The data that we will be working on in this lab is an expression matrix with genes 
in rows and samples in columns. We have essentially just collected files with mRNA counts 
per gene for a number of samples and merged them into a matrix.
...
In this lab, we will only explore a few of these steps with a focus on quality control and differential 
expression analysis.

## Analysis pipeline

In this second part of the lab we will use the DESeq2 R package to demonstrate an analysis workflow
of bulk RNA-seq data.

### Pre-processing 

Filtering out low abundant genes is not a requirement to run DESeq2 functions but can be useful for 
two reasons: we can reduce the memory size of our DESeq2 data object and increase the speed of 
transformation and testing functions within our analysis workflow. Note that when your run a DE test
using DESeq2, a more strict filtering is automatically applied before running the test that you can 
read more about [here](http://bioconductor.org/packages/release/bioc/vignettes/DESeq2/inst/doc/DESeq2.html#indfilt).

Q4. Exclude all genes that have a total count value across all samples of $<=100$. Use the skills you learned in Lab 1.
...
DESeq2 takes many different data types as inut, but since we have a count matrix we'll use 
the `DESeqDataSetFromMatrix()` function to create our DESeq2 object.

When we create our object we will add the count data and metadata, but we will also specify 
a design formula. This formula expresses how the counts for each gene depend on the variables 
in colData. This can be changed later into more complex designs, but here we will 
simply tells DESeq2 that the counts are grouped by breast cancer subtype.
...
If you now print the `dds` object, you will obtain information about the contents of the 
object. We can for example see that there are 30107 genes and 60 samples and we also get 
a few example names for each type.
...
We can use these sizefactors to normalize our samples by dividing the raw count with the size factor. This will
...
Q5. Make a boxplot for each sample showing the distribution of normalized counts.

Hint: You already have the code to do this from the first part of the lab. Look at Q2 and Q3 as well as the code 
used to prepare the `counts_long` data.frame. Here you will need to replace the `counts` matrix with the 
normalized counts.

```{r boxplot_normalized, eval=TRUE, echo=TRUE, fig.width=12, fig.height=5}

counts_long <- setNames(reshape2::melt(log(counts(dds, normalized = TRUE) + 0.5)), nm = c("gene", "sample_id", "log_count"))
counts_long$subtype <- metadata[counts_long$sample_id, ]$subtype

ggplot(counts_long, aes(sample_id, log_count)) +
  geom_boxplot() +
  facet_wrap(vars(subtype), scales = "free", ncol = 6) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
If you managed to do this correctly you should be able to see that the distributions are now more
...
that you are working with. For example, it might be interesting to know whether 
some groups exhibit greater within-group variability than other or to check how samples 
are related to each other based on expression similarity. You might even want to use 
a clustering approach based on similarities of gene expression profiles to define new 
subtypes. This is actually how our subtypes were defined within the TCGA!

Before we move on to data exploration it is often beneficial to transform 
the count data first. Otherwise, highly expressed genes tend to dominate the result. 

One frequently used method is the variance-stabilizing transformation (VST),
which is used to disentangle the mean-variance relationship seen in count data.
...
A heatmaps is a type of plot that can be useful to obtain an overview of pairwise relationships 
between many samples at the same time. Here we have 60 samples which is a decent amount of samples 
and it could be useful to understand if these are correlated with each other at the gene 
expression level.

Using our normalized counts (VST transformed), we can compute correlation scores between each pair
of samples and visualize these correlationns using the code below:
...
purple color represents a pair of samples with low correlation. We can see that there are at least three larger blocks 
of samples with higher similarity, but right now we can only make a guess for why this is the case.

Let's add a new component to this heatmap to show some of our sample metadata using the `annotation_col` argument 
in `pheatmap`. What we want to do here is to add a bar on top of the heatmap to show the cancer subtype for each sample. 
...
One thing that becomes quite clear after looking at this heatmap is that even though there is
a lot of heterogeneity across samples, there are similarities within the respective subtypes as well. 
Next, we want to know what genes are driving this separation of subtypes at a transcriptional level.

### Differential expression

When we conduct DE analysis with DESeq2, we are free to set up quite complex designs to adress 
our question of interest. For example, we could be interested in differential expression
between a subtype and our Normal control or one subtype against all other subtypes. Or if we 
have access to additional clinical data, we could for example design our test to adress 
differences in gene expression due to genotype, age, gender, BMI, treatments etc.

Our current design allows us to do pairwise comparisons between subtypes and we can
check the coefficients of our model using `resultsNames`.
...
results for a specific comparison, we can use `results` command. For this simple example
we set the contrast using a vector of length 3 where the first element is the name of a 
factor in the design formula, the second element is the numerator and the third element 
is the denominator. We have also added a few filtering steps to remove genes with 
an absolute log2 foldchange lower than 2, adjusted p-value higher than 0.01 and then
we have arranged the results by increasing adjusted p-values to have the most significant 
hits in the top of our table.
...
Q5. Select the top 10 most differentially up- and down-regulated genes with respect to adjusted p-values from the `res_summarized` data.frame. You should be able to solve this problem programatically.
...
If we want to obtain DEGs that are specific to a subtype we could make pairwise comparisons between that specific
subtype and all other subtypes.
...
We do find quite many subtype specific genes in our data, but even the top most 
differentially expressed genes have a quite high within group variability. Cancer subtypes 
are heterogeneous and remember that these are bulk RNA-seq data which means that we are investigating 
the averaged expression over a whole tissue piece. 

Cancer cells reside and proliferate within a microenvironment made up of "normal" cells. 
This could for example be cells that build up connective tissues and blood vessels or immune cells 
that are trying to fight and eliminate the tumor. Although cells in the tumor
microenvironment are not malignant, they can be manipulated by the cancer cells to suppress the immune 
response and promote disease progression. Understanding the intricate details of cell-cell crosstalk 
and the architecture of the microenvironment is important in the quest to find new therapeutic strategies.
This limitation with bulk RNA-seq analysis is to a large extent the reason why single-cell and spatial 
methods have taken over the field of genomics and gained so much traction in the last decade.
