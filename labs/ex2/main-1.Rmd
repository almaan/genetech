...
This is a general pipeline for a bulk RNA-seq analysis workflow. When you have
raw sequencing reads, filter them based on quality and map them back to the
reference genome, or you use de novo assembly tool to construct the genome.

In the next step you filter the mapping results and remove PCR duplicates.
Normalisation is needed for the reads because there is differences in the amount
of reads, samples, mapping bias etc. Differential gene expression (DGE) analysis,
clustering and network analysis usually follow normalisation.
...
distributions of the raw counts we need to log the counts. Next we’ll use box
plots to check the distribution of the read counts on the log2 scale.
