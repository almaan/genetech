...
Q4. Exclude all genes that have a total count value across all samples of $<=10$. Use the skills you learned in Lab 1.
...
dds$subtype <- relevel(dds$subtype, ref = "Normal_NT")
...
counts_long <- setNames(reshape2::melt(log10(counts(dds, normalized = TRUE) + 1)), nm = c("gene", "sample_id", "log10_count"))
counts_long$subtype <- metadata[counts_long$sample_id, ]$subtype
...
If you managed to do this correctly you should be able to see that the distributions are more
...
Another plot that is related to the previous heatmap is the PCA plot. This type of plot
can be particularly useful for visualizing the overall effect of both technical 
and biological covariates as well as batch effects. Here we do not have access to 
technical data, but we if we color the PCA by subtype we can see that the samples 
group by breast cancer subtype as expected.

```{r pca}

plotPCA(vst.data, intgroup = "subtype")
...
For example, "group_Basal_vs_Normal", contains the DE result comparing Normal against Basal.
...
results for a specific comparison, we can use results command:

```{r, eval= TRUE, echo=TRUE}

res <- results(dds, contrast = c("subtype", "LumA_TP", "Normal_NT"), tidy = TRUE)
res <- res %>% 
  dplyr::arrange(padj) %>%
  dplyr::filter(log2FoldChange > 2, padj < 0.01)

```

```{r}

gg <- data.frame(subtype = colData(dds)$subtype, val = dds@assays@data$counts["CDC25C", ])
