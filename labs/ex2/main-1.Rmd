...
Using the VST transformed counts, we can create a correlation heatmap to visualize relationships between samples at the gene expression level:

```{r, eval= TRUE, echo=TRUE, fig.height=9, fig.width=9}

pc <- prcomp(t(assay(vst.data)))
sampleDistMatrix <- cor(t(pc$x))
rownames(sampleDistMatrix) <- vst.data$Subtype_mRNA
